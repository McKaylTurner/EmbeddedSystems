...
String redState = "off"; 
String blinkState = "off";             // Initialize a variable for tracking the state of the LED
String color = "";
...
void loop() { // ** Create a web server - Adapted from http://www.esp8266learning.com/wemos-webserver-example.php
  ...
  if (client) {                           // If a client is connected, wait until it sends some data
    ...
    if (request.indexOf("/green=on") != -1) {   // If the request is for the page "/green=on"
      ...
      digitalWrite(yellow, LOW);            //   then set the ledPin to high (turn on)
    ...
    if (request.indexOf("/yellow=on") != -1) {   // If the request is for the page "/yellow=on"
      ...
      digitalWrite(green, LOW); 
      digitalWrite(red, LOW);              //   then set the ledPin to high (turn on)
    ...
    if (request.indexOf("/backUp=on") != -1) {   // If the request is for the page "/green=on"           //   then set the ledPin to high (turn on)
      backUpState = "on"; 
      digitalWrite(yellow, LOW);
      digitalWrite(green, LOW);                       //   and track the state as "on"
    }
    if (request.indexOf("/backUp=off") != -1) {   // If the request is for the page "/green=on"           //   then set the ledPin to high (turn on)
      backUpState = "off";  
      digitalWrite(yellow, LOW); 
      digitalWrite(red, LOW);
      digitalWrite(green, LOW);  
      redBlink = "off";                 //   and track the state as "on"
    }
    // ** End Adapted Code - This is the end of the code that was adapted from www.esp8266learning.com
...
//green
   ...
   //yellow
    ...
    //backUp
    client.print("<h2> Back Up </h2>");
    client.println("<label class='switch'>");
    client.print("  <input type=\"checkbox\" ");
    if (backUpState == "on") {
      client.print("checked ");
    }
    client.print("onchange=\"window.location.href='/backUp=");
    if (backUpState == "on") {
      client.print("off");
    } else {
      client.print("on");
...
if ((backUpState == "on") && (blinkTimer < millis())) {            // If it is time to change
    blinkTimer = millis() + blinkFreq;    // Set a new trigger time
    digitalWrite(red, HIGH);
    delay(1000);
    digitalWrite(red, LOW);
